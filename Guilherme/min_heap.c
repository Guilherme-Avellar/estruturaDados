...
static int indice_filho_esq(heap *h, int id_p);
static int indice_filho_dir(heap *h, int id_p);
static int inserir_fim_heap(heap* h, unsigned int v);
static void min_heapify_heap(heap *h, int id_p);
static int indice_pai(heap *h, int id_f);
...
//A:> Faltaram alguns testes com relação aos valores de h, id_p e id_f
static int indice_filho_esq(heap *h, int id_p) { // id_p: índice de um pai
    return (2*id_p) + 1;
}

static int indice_filho_dir(heap *h, int id_p) { // id_p: índice de um pai
    return (2*id_p) + 2;
}

static int indice_pai(heap *h, int id_f) {     // id_f: índice de um filho
    //A:> Deveria ser => return (id_f - 1) / 2;
    return (id_f - 2) / 2;
}
...
static void min_heapify(heap *h, int id_p) {          // id_p: índice do elemento pai
